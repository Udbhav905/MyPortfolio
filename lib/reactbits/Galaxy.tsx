...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL not supported, Galaxy background disabled", e);
      return;
    }
    ...
    return () => {
      ...
      try {
        if (ctn && gl && gl.canvas && ctn.contains(gl.canvas as HTMLCanvasElement)) {
          ctn.removeChild(gl.canvas as HTMLCanvasElement);
        }
      } catch (err) {
        console.warn("Galaxy cleanup canvas failed", err);
      }
      try {
